...
class MySendMail {
    ...
    public function sendMail() {
        ...
        print_r( $command ); echo "  <- command 22222\n";
    ...
    protected function sendCommandSecurity($command, $code) {
        echo 'sendCommandSecurity Send command:' . $command . ',expected code:' . $code . '<br />' . "\n";
        try {
    ...
    private function socketSecurity() {
        ...
        $fp = stream_socket_enable_crypto($this->_socket, true, STREAM_CRYPTO_METHOD_TLSv1_2_CLIENT);    // yyq change, old is: STREAM_CRYPTO_METHOD_SSLv23_CLIENT
        if (!$fp) {
